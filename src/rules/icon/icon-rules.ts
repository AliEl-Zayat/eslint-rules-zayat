...
function findColorAttributeIssues(node: TSESTree.JSXElement): ColorIssue[] {
...
function getComponentName(declaration: TSESTree.ExportDefaultDeclaration['declaration']): string | null {
...
export const singleSvgPerFile = {
	meta: {
		...
		docs: {
			...
			url: 'https://github.com/AliEl-Zayat/vertex-era-rules/blob/main/docs/rules/single-svg-per-file.md',
		},
		schema: [],
		messages: {
			multipleSvgs:
...
export const svgCurrentcolor = {
	meta: {
		...
		docs: {
			...
			category: 'Best Practices',
			recommended: 'error' as const,
			url: 'https://github.com/AliEl-Zayat/vertex-era-rules/blob/main/docs/rules/svg-currentcolor.md',
		},
	...
	create(context: {
		report: (descriptor: {
			...
			messageId: string;
			data: Record<string, unknown>;
			fix?: (fixer: RuleFixer) => ReturnType<RuleFixer['replaceText']> | null;
		}) => void;
	}) {
		return {
			JSXElement(node: TSESTree.JSXElement) {
...
export const memoizedExport = {
	meta: {
		...
		docs: {
			...
			category: 'Best Practices',
			recommended: 'error' as const,
			url: 'https://github.com/AliEl-Zayat/vertex-era-rules/blob/main/docs/rules/memoized-export.md',
		},
		fixable: 'code' as const,
		schema: [],
		messages: {
	...
	create(context: {
		report: (descriptor: {
			...
			messageId: string;
			data: Record<string, unknown>;
			fix?: (fixer: RuleFixer) => (ReturnType<RuleFixer['replaceText']> | ReturnType<RuleFixer['replaceTextRange']>)[];
		}) => void;
					...
					if (componentName) {
						ctx.report({
							...
							fix(fixer) {
								...
								const fixes: (ReturnType<RuleFixer['replaceText']> | ReturnType<RuleFixer['replaceTextRange']>)[] = [];
