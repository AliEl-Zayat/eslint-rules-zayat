...
export const booleanNamingConvention = createRule<[RuleOptions], 'booleanNaming'>({
	...
	create(context) {
		...
		function isBooleanLiteral(node: TSESTree.Node | null): boolean {
			...
			if (node.type === 'Literal' && typeof (node as TSESTree.Literal).value === 'boolean') {
				return true;
			}

			if (node.type === 'UnaryExpression' && (node as TSESTree.UnaryExpression).operator === '!') {
				return true;
			}
